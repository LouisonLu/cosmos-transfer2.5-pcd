...
from cosmos_transfer2._src.transfer2.inference.inference_pipeline_frame0_spatial_lock import (
    ControlVideo2WorldInferenceFrame0SpatialLock,
)
...
FRAME0_SPATIAL_LOCK_EXPERIMENT = "transfer2_singleview_frame0_spatial_lock_pcd_rgb_image_context_example"
FRAME0_SPATIAL_LOCK_CONFIG_FILE = "cosmos_transfer2/singleview_frame0_spatial_lock_config.py"


class Control2WorldInferenceFrame0SpatialLock:
    def __init__(
        self,
        args: SetupArguments,
        ...
        if registered_exp_name == FRAME0_SPATIAL_LOCK_EXPERIMENT and args.config_file != FRAME0_SPATIAL_LOCK_CONFIG_FILE:
            raise ValueError(
                "Frame0 spatial-lock inference requires the matching config file. "
                f"Please add: --config-file {FRAME0_SPATIAL_LOCK_CONFIG_FILE}"
            )

        # Initialize the inference pipeline - same class for both distilled and non-distilled
        self.inference_pipeline = ControlVideo2WorldInferenceFrame0SpatialLock(
            # pyrefly: ignore [bad-argument-type]

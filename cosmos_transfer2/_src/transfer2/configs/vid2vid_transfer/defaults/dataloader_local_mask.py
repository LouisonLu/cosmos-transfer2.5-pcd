...
"""Isolated local dataloader configs for masked-image-context experiments."""
...
def register_dataloader_local_mask() -> None:
    """Register mask-specific local dataloader configs without touching the default ones."""
    cs = ConfigStore()

    dataset_depth_mask = L(SingleViewTransferDatasetMask)(
    ...
    cs.store(
        ...
        name="example_singleview_train_data_depth_mask",
        node=L(get_generic_dataloader)(
            dataset=dataset_depth_mask,
            sampler=L(get_sampler)(dataset=dataset_depth_mask) if dist.is_initialized() else None,
            batch_size=1,
            drop_last=True,
            num_workers=4,
            pin_memory=True,

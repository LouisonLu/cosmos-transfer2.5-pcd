...
Example usage:
    dataset = SingleViewTransferDatasetVisiblePreserve(
...
class SingleViewTransferDatasetVisiblePreserve(Dataset):
    ...
    def __init__(
        ...
        **kwargs,  # Accept extra params for config compatibility (like MultiviewTransferDataset)
        ...
        log.info(f"Initialized SingleViewTransferDatasetVisiblePreserve with {len(self.video_paths)} videos")
    ...
    def __str__(self) -> str:
        return f"SingleViewTransferDatasetVisiblePreserve: {len(self.video_paths)} videos from {self.dataset_dir}"
    ...
    def _mask_image_context(self, frame: np.ndarray) -> np.ndarray:
        ...
        keep = self._get_image_context_keep_mask(frame.shape[0], frame.shape[1])
        if keep is not None:
    ...
    def _get_image_context_keep_mask(self, height: int, width: int) -> np.ndarray | None:
        """Return the deterministic visible-region mask for the configured mask mode."""
        h, w = height, width
        keep_ratio = self.image_context_keep_ratio

        if self.image_context_mask_reference_path is not None:
            return self._get_reference_keep_mask(h, w)
        ...
        if self.image_context_mask_mode in {"waymo", "front_rear_luma_erp"}:
            return self._get_waymo_keep_mask(h, w)
        if self.image_context_mask_mode == "random_pixels":
            return np.random.random((h, w)) < keep_ratio

        return None

    def _old_mask_image_context_unused(self, frame: np.ndarray) -> np.ndarray:
        """Deprecated V1 implementation kept only for local diff readability."""
        masked = np.full_like(frame, np.clip(self.image_context_mask_fill_value, 0, 255))
        h, w = frame.shape[:2]
        keep_ratio = self.image_context_keep_ratio
        ...
        for retry in range(max_retries):
            ...
            try:
                ...
                if self.use_image_context and self.image_context_from_rgb_first_frame:
                    ...
                    keep_mask = None
                    if self.mask_image_context:
                        keep_mask = self._get_image_context_keep_mask(first_frame.shape[0], first_frame.shape[1])
                    ...
                    if keep_mask is not None:
                        mask = np.zeros((self.sequence_length, first_frame.shape[0], first_frame.shape[1]), dtype=np.uint8)
                        mask[0] = keep_mask.astype(np.uint8)
                        data["visible_region_mask"] = torch.from_numpy(mask).unsqueeze(0)
                ...
                # Keep the visible-region mask aligned with the augmented target video.
                if "visible_region_mask" in data:
                    _, t, h, w = data["video"].shape
                    visible_mask = data["visible_region_mask"].float()
                    if visible_mask.shape[-2:] != (h, w):
                        ...
                        resized_mask = visible_mask
                    if resized_mask.shape[1] != t:
                        fixed_mask = torch.zeros((1, t, h, w), dtype=resized_mask.dtype)
                        fixed_mask[:, : min(t, resized_mask.shape[1])] = resized_mask[
                            :, : min(t, resized_mask.shape[1])
                        ]
                        resized_mask = fixed_mask
                    data["visible_region_mask"] = resized_mask.clamp(0, 1).to(torch.float32)
...
if __name__ == "__main__":
    ...
    Usage:
        PYTHONPATH=. python cosmos_transfer2/_src/transfer2/datasets/local_datasets/singleview_dataset_visible_preserve.py
    """
    import sys

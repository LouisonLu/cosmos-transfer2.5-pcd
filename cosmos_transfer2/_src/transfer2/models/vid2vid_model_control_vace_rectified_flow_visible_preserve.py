...
class ControlVideo2WorldRectifiedFlowVisiblePreserveConfig(Video2WorldModelRectifiedFlowConfig):
    ...
    visible_region_keep_noise_scale: float = 0.01
    visible_region_boundary_noise_scale: float = 0.35
    visible_region_loss_weight: float = 4.0
    missing_region_loss_weight: float = 1.0
    boundary_region_loss_weight: float = 2.0
    boundary_width_latent: int = 2


class ControlVideo2WorldModelRectifiedFlowVisiblePreserve(Video2WorldModelRectifiedFlow):
    """
    ImaginaireModel instance of the VACE-styled controlnet for training.
    """

    def __init__(self, config: ControlVideo2WorldRectifiedFlowVisiblePreserveConfig, *args, **kwargs):
    ...
    def _get_latent_visible_mask(
        self, data_batch: dict[str, torch.Tensor], latent_shape: torch.Size
    ) -> torch.Tensor | None:
        visible_mask = data_batch.get("visible_region_mask")
        if visible_mask is None:
            return None

        if visible_mask.ndim == 4:
            visible_mask = visible_mask.unsqueeze(0)
        visible_mask = visible_mask.to(**self.tensor_kwargs_fp32)
        visible_mask = F.interpolate(visible_mask, size=latent_shape[-3:], mode="nearest")
        return visible_mask.clamp(0.0, 1.0)

    def _split_mask_for_context_parallel(
        self,
        mask_B_1_T_H_W: torch.Tensor | None,
        condition,
        original_latent_shape: torch.Size,
    ) -> torch.Tensor | None:
        if mask_B_1_T_H_W is None:
            return None

        cp_group = self.get_context_parallel_group()
        cp_size = 1 if cp_group is None else cp_group.size()
        if not condition.is_video or cp_size <= 1:
            return mask_B_1_T_H_W

        use_spatial_split = cp_size > original_latent_shape[2] or original_latent_shape[2] % cp_size != 0
        after_split_shape = find_split(original_latent_shape, cp_size) if use_spatial_split else None
        if use_spatial_split:
            mask_B_1_T_H_W = rearrange(mask_B_1_T_H_W, "B C T H W -> B C (T H W)")
        mask_B_1_T_H_W = broadcast_split_tensor(mask_B_1_T_H_W, seq_dim=2, process_group=cp_group)
        if use_spatial_split:
            mask_B_1_T_H_W = rearrange(
                mask_B_1_T_H_W,
                "B C (T H W) -> B C T H W",
    ...
    def _get_frame0_region_masks(self, visible_mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        frame_mask = torch.zeros_like(visible_mask)
        if visible_mask.shape[2] == 0:
            return frame_mask, frame_mask, frame_mask

        visible_frame = visible_mask[:, :, :1]
        width = max(0, int(self.config.boundary_width_latent))
        if width > 0:
            b, c, t, h, w = visible_frame.shape
            visible_2d = visible_frame.reshape(b * t, c, h, w)
            kernel = width * 2 + 1
            dilated = F.max_pool2d(visible_2d, kernel_size=kernel, stride=1, padding=width)
            eroded = 1.0 - F.max_pool2d(1.0 - visible_2d, kernel_size=kernel, stride=1, padding=width)
            boundary_2d = (dilated - eroded).clamp(0.0, 1.0)
            boundary = boundary_2d.reshape(b, c, t, h, w)
        else:
            boundary = torch.zeros_like(visible_frame)

        visible_core = (visible_frame * (1.0 - boundary)).clamp(0.0, 1.0)
        missing = ((1.0 - visible_frame) * (1.0 - boundary)).clamp(0.0, 1.0)

        visible_full = torch.zeros_like(visible_mask)
        missing_full = torch.zeros_like(visible_mask)
        boundary_full = torch.zeros_like(visible_mask)
        visible_full[:, :, :1] = visible_core
        missing_full[:, :, :1] = missing
        boundary_full[:, :, :1] = boundary
        return visible_full, missing_full, boundary_full
    ...
    def forward(self, data_batch: dict[str, torch.Tensor]) -> tuple[dict[str, torch.Tensor], torch.Tensor]:
        ...
        visible_mask = self._get_latent_visible_mask(data_batch, x0_B_C_T_H_W.shape)
        if visible_mask is not None:
            visible_core, missing_region, boundary_region = self._get_frame0_region_masks(visible_mask)
            noise_scale = torch.ones_like(visible_mask)
            noise_scale = noise_scale * (1.0 - visible_core) + visible_core * self.config.visible_region_keep_noise_scale
            noise_scale = (
                noise_scale * (1.0 - boundary_region)
                + boundary_region * self.config.visible_region_boundary_noise_scale
            )
            epsilon_B_C_T_H_W = epsilon_B_C_T_H_W * noise_scale
        else:
            visible_core = missing_region = boundary_region = None
        ...
        original_latent_shape = x0_B_C_T_H_W.shape
        ...
        visible_core = self._split_mask_for_context_parallel(visible_core, condition, original_latent_shape)
        missing_region = self._split_mask_for_context_parallel(missing_region, condition, original_latent_shape)
        boundary_region = self._split_mask_for_context_parallel(boundary_region, condition, original_latent_shape)
        ...
        mse = (vt_pred_B_C_T_H_W - vt_B_C_T_H_W) ** 2
        time_weights_B = self.rectified_flow.train_time_weight(timesteps, self.tensor_kwargs_fp32)
        per_instance_loss = torch.mean(mse, dim=list(range(1, mse.dim())))
        flow_loss = torch.mean(time_weights_B * per_instance_loss)

        visible_loss = torch.zeros((), **self.tensor_kwargs_fp32)
        missing_loss = torch.zeros((), **self.tensor_kwargs_fp32)
        boundary_loss = torch.zeros((), **self.tensor_kwargs_fp32)
        if visible_core is not None:
            visible_loss = self._masked_mean(mse, visible_core)
            missing_loss = self._masked_mean(mse, missing_region)
            boundary_loss = self._masked_mean(mse, boundary_region)

        loss = (
            flow_loss
            + self.config.visible_region_loss_weight * visible_loss
            + self.config.missing_region_loss_weight * missing_loss
            + self.config.boundary_region_loss_weight * boundary_loss
        )
        output_batch = {
            "x0": x0_B_C_T_H_W,
            "xt": xt_B_C_T_H_W,
            ...
            "edm_loss": flow_loss,
            "visible_region_loss": visible_loss,
            "missing_region_loss": missing_loss,
            "boundary_region_loss": boundary_loss,
            "loss": loss,
            "timesteps": timesteps,

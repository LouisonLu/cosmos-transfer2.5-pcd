...
class ControlVideo2WorldModelRectifiedFlowParticalHardlock(Video2WorldModelRectifiedFlow):
    ...
    def get_velocity_fn_from_batch(
        ...
        partial_hardlock_mask = data_batch.get("partial_hardlock_mask", None)
        if partial_hardlock_mask is not None:
            condition = self._apply_partial_hardlock_mask(condition, partial_hardlock_mask)
            uncondition = self._apply_partial_hardlock_mask(uncondition, partial_hardlock_mask)
    ...
    def _apply_partial_hardlock_mask(self, condition, partial_hardlock_mask: torch.Tensor):
        """Limit the normal conditional-frame hardlock to the requested spatial mask."""
        condition_mask = condition.condition_video_input_mask_B_C_T_H_W
        if condition_mask is None:
            return condition

        mask = partial_hardlock_mask.to(device=condition_mask.device, dtype=condition_mask.dtype)
        if mask.shape[1] != 1:
            mask = mask[:, :1]
        if mask.shape[2:] != condition_mask.shape[2:]:
            raise ValueError(
                "partial_hardlock_mask shape does not match condition mask: "
                f"{tuple(mask.shape)} vs {tuple(condition_mask.shape)}"
            )

        kwargs = condition.to_dict(skip_underscore=False)
        kwargs["condition_video_input_mask_B_C_T_H_W"] = condition_mask * mask

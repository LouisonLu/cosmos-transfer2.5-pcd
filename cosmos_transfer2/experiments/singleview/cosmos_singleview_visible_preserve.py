...
"""Visible-region-preserving single-view post-training configs.

This module is intentionally separate from ``cosmos_singleview_example.py`` so
V2 experiments can be removed without touching the V1 baseline.
"""
...
transfer2_singleview_visible_preserve_pcd_rgb_image_context_example = dict(
    defaults=[
        DEFAULT_BASE_EXPERIMENT,
        {"override /model": "fsdp_control_vace_rectified_flow_visible_preserve"},
        {"override /data_train": "example_singleview_train_data_depth_visible_preserve"},
    ...
    job=dict(
        ...
        name="transfer2_singleview_visible_preserve_pcd_rgb_image_context_example",
    ),
    checkpoint=dict(
        save_iter=1000,
    ...
    model=dict(
        config=dict(
            ...
            visible_region_keep_noise_scale=0.01,
            visible_region_boundary_noise_scale=0.35,
            visible_region_loss_weight=4.0,
            missing_region_loss_weight=1.0,
            boundary_region_loss_weight=2.0,
            boundary_width_latent=2,
            net=dict(
...
for _item in [transfer2_singleview_visible_preserve_pcd_rgb_image_context_example]:
